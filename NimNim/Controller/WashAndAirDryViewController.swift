...
class WashAndAirDryViewController: UIViewController,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,SpecialNotesCollectionViewCellDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,NoofClothesCollectionViewCellDelegate,WashAndFoldPreferencesCollectionViewCellDelegate{
    ...
    @objc func keyboardWillShow(notification: NSNotification) {
        ...
        if let keyboardSize = (notification.userInfo?[UIResponder.keyboardFrameBeginUserInfoKey] as? NSValue)?.cgRectValue {
            if !isHeightAdded {
                ...
                washAndAirDryCollectionView.contentInset = UIEdgeInsets(top: washAndAirDryCollectionView.contentInset.top, left: washAndAirDryCollectionView.contentInset.left, bottom: addedHeight, right: washAndAirDryCollectionView.contentInset.right)
                isHeightAdded = true
            }
        }
    }
    ...
    @objc func keyboardWillHide(notification: NSNotification) {
        if isHeightAdded {
            washAndAirDryCollectionView.contentInset = UIEdgeInsets(top: washAndAirDryCollectionView.contentInset.top, left: washAndAirDryCollectionView.contentInset.left, bottom: 0, right: washAndAirDryCollectionView.contentInset.right)
            isHeightAdded = false
        }
    }
    ...
    @IBAction func infoIconTapped(_ sender: Any) {
        ...
        if let price = serviceModel?.price{
                  jnnvc.descriptionValue = "Forget per piece pricing, tap here to choose this option and pay only $3 per pound of laundry for your order"
        }
